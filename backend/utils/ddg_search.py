...
def discover_job_urls(site_domain: str, job_title: str, location: str, max_results: int = 15) -> List[str]:
    ...
    logger.info(f"Querying DuckDuckGo for: {query} [timelimit='m', region='uk-en' applied]")
    urls = []
    try:
        with DDGS() as ddgs:
            # We fetch up to max_results text items, restricting to the last month ('m') and UK region
            results = ddgs.text(query, timelimit='m', max_results=max_results, region='uk-en')
            for r in results:
                url = r.get("href")
                if url:
                    # Clean/normalize URL
